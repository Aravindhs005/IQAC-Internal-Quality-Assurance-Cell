...
if (isset($_POST['data_id'], $_POST['expected'],$_POST['actual'])) {
    // Sanitize the input
    ...
    $queryUpdateActual = "UPDATE data SET actual = $actual,expected=$expected WHERE data_id = $dataId";
    ...
    if ($resultUpdateActual) {
        ...
        if ($resultFetchExpected && mysqli_num_rows($resultFetchExpected) > 0) {
            ...
            if (array_key_exists($measurement_id, $formulas)) {
                ...
                $approvetrue="UPDATE data SET approved = CASE WHEN approved = TRUE THEN FALSE ELSE TRUE END WHERE data_id = $dataId";
                $runtheapprove= mysqli_query($con,$approvetrue);
