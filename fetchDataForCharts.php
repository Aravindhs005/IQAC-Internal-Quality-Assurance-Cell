...
$stmt = $con->prepare("SELECT user_id, COUNT(actual) AS filled_count FROM data GROUP BY user_id");
$stmt->execute();
...
if ($result->num_rows > 0 && $res->num_rows > 0) {
    ...
    while(($row = $result->fetch_assoc()) && ($row2 = $res->fetch_assoc())) {
        ...
        $filled_count_out_of_91 = min($filled_count, 91); // Limit filled count to 91
        
        // Append to the arrays
        $labels[] = $department_name;
        $values[] = $filled_count_out_of_91;
    }
}
